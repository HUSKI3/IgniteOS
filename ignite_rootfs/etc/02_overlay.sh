...
DEFAULT_OVERLAY_DIR="/tmp/ignite/overlay"
DEFAULT_UPPER_DIR="/tmp/ignite/rootfs"
DEFAULT_WORK_DIR="/tmp/ignite/work"
...
for DEVICE in /dev/* ; do
  ...
  if [ -d $DEVICE_MNT/ignite/rootfs -a -d $DEVICE_MNT/ignite/work ] ; then
    # folder
    echo -e "  Found \\e[94m/ignite\\e[0m folder on device \\e[31m$DEVICE\\e[0m."
    touch $DEVICE_MNT/ignite/rootfs/ignite.pid 2>/dev/null
    if [ -f $DEVICE_MNT/ignite/rootfs/ignite.pid ] ; then
      # read/write mode
      echo -e "  Device \\e[31m$DEVICE\\e[0m is mounted in read/write mode."

      rm -f $DEVICE_MNT/ignite/rootfs/ignite.pid

      OVERLAY_DIR=$DEFAULT_OVERLAY_DIR
      OVERLAY_MNT=$DEVICE_MNT
      UPPER_DIR=$DEVICE_MNT/ignite/rootfs
      WORK_DIR=$DEVICE_MNT/ignite/work
    else
      # read only mode
      echo -e "  Device \\e[31m$DEVICE\\e[0m is mounted in read only mode."

      OVERLAY_DIR=$DEVICE_MNT/ignite/rootfs
      OVERLAY_MNT=$DEVICE_MNT
      UPPER_DIR=$DEFAULT_UPPER_DIR
      WORK_DIR=$DEFAULT_WORK_DIR
    fi
  elif [ -f $DEVICE_MNT/ignite.img ] ; then
    #image
    echo -e "  Found \\e[94m/ignite.img\\e[0m image on device \\e[31m$DEVICE\\e[0m."

    mkdir -p /tmp/mnt/image
    IMAGE_MNT=/tmp/mnt/image

    LOOP_DEVICE=$(losetup -f)
    losetup $LOOP_DEVICE $DEVICE_MNT/ignite.img

    mount $LOOP_DEVICE $IMAGE_MNT
    if [ -d $IMAGE_MNT/rootfs -a -d $IMAGE_MNT/work ] ; then
      touch $IMAGE_MNT/rootfs/ignite.pid 2>/dev/null
      if [ -f $IMAGE_MNT/rootfs/ignite.pid ] ; then
        # read/write mode
        echo -e "  Image \\e[94m$DEVICE/ignite.img\\e[0m is mounted in read/write mode."

        rm -f $IMAGE_MNT/rootfs/ignite.pid
        ...
        # read only mode
        echo -e "  Image \\e[94m$DEVICE/ignite.img\\e[0m is mounted in read only mode."
